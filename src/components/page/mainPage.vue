                        ...
                        <el-card shadow="hover" :body-style="{padding: '0px'}">
                            <div class="grid-content grid-con-1">
                                ...
                                <div class="grid-cont-right">
                                    <div id="connected_car" class="grid-num">{{connected_Num}}</div>
                                    <div>在线车辆数量</div>
                                </div>
                            </div>
                        </el-card>
                    ...
                    <el-col :span="8">
                        <el-card shadow="hover" :body-style="{padding: '0px'}">
                            <div class="grid-content grid-con-2">
                                ...
                                <div class="grid-cont-right">
                                    <div class="grid-num" >{{park_Num}}</div>
                                    <div>园区数量</div>
                                </div>
                            </div>
                        </el-card>
...
export default {
    ...
    methods: {
        ...
        fetch(){
            ...
            this.$http.get('http://'+this.url+'/api/v2/info/car/connected',{params:{
                    page:0,
                    size:0
                }
            }).then(response=>{
                this.connected_Num = response.body.attach.length;
            }, response=>{
                this.connected_Num = 0;
            });

            this.$http.get('http://'+this.url+'/api/v2/parks').then(response=>{
                this.park_Num = response.body.attach.length;
            }, response=>{
                this.park_Num = 0;
            });
        }
